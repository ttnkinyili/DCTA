# Walkthrough: Variance-Weighting + Beyond the Perimeter Combination

## What Was Done

### 1. Reviewed Both Papers in Full
- **paper_beyond_the_perimeter.md** — 428 lines, ~8,700 words (literature review article)
- **paper_variance_weighting_fusion.md** — 664 lines, ~10,300 words (research article)
- Combined original total: ~19,000 words

### 2. Produced Review Analysis
- Assessed the reviewer's salami-slicing comment (see Part II below)
- Verdict: comment is **substantially valid** — both papers share the same central thesis, proposed solution, and identical mathematical formulations

### 3. Created Combined Paper
- **paper_variance_weighting_beyond_the_perimeter.md** — 608 lines, ~7,900 words

## How the Papers Were Combined

| Section | Source | Treatment |
|:---|:---|:---|
| Introduction (§I) | Both papers | Merged implicit trust problem (BtP) with signal reliability problem (VWF); retained motivating scenario from VWF |
| Critical Analysis (§II) | BtP §2–6 | **Condensed from ~9,000 to ~3,500 words** — each paradigm gets one focused subsection |
| Related Work (§III) | VWF §II + §VII | Merged and de-duplicated |
| Proposed Approach (§IV) | VWF §III + BtP §7 | Mathematical formulations retained once; formal properties from BtP §7.4 added; architectural integration table from BtP preserved |
| Experimental Setup (§V) | VWF §IV | Retained in full |
| Results (§VI) | VWF §V | Retained in full (ablation, sensitivity, latency) |
| Discussion (§VII) | Both | Merged; de-duplicated limitations |
| References | Both | 50 references, de-duplicated, IEEE numbered format |

## Key Metrics

| Metric | Original (2 papers) | Combined |
|:---|:---:|:---:|
| Total words | ~19,000 | ~7,900 |
| Reduction | — | **58%** |
| Tables | ~20 (with duplicates) | 16 (unique) |
| References | ~57 (with duplicates) | 50 (de-duplicated) |
| Sections | 16 major sections | 8 major sections |

## Content Mapping Summary

| Content Block | Original Location | Combined Paper Location |
|:---|:---|:---|
| Implicit trust period problem | BtP §1 | §I-A (condensed) |
| Signal reliability problem | VWF §I-A | §I-B |
| Motivating failure scenario | VWF §I-B | §I-C (retained as-is) |
| 5-paradigm critique | BtP §2–6 (~9,000 words) | §II-A through §II-F (~3,500 words) |
| Unified failure mapping (Table 1) | BtP Table 1 | Table I (retained) |
| Variance weighting formula | Both (duplicated) | §IV-A (once only) |
| DS combination rule | Both (duplicated) | §IV-D (once only) |
| Pignistic transformation | Both (duplicated) | §IV-D (once only) |
| Temporal decay formula | Both (duplicated) | §IV-E (once only) |
| Formal properties (anti-spoofing, self-calibration) | BtP §7.4 | §IV-F |
| Architectural integration table | BtP Table 3 | Table VIII |
| 6-scenario experimental evaluation | VWF §IV–V | §V–VI (retained in full) |
| Ablation study | VWF §V-E | §VI-D (retained in full) |
| α sensitivity analysis | VWF §V-F | §VI-E (retained in full) |
| Latency benchmarking | VWF Table XI | Table XV (retained in full) |
| Cold-start handling | VWF §VI-A | §VII-A |
| Known-bad vs. uncertain | VWF §VI-C | §VII-B |
| Limitations | Both (overlapping) | §VII-D (merged, de-duplicated) |
